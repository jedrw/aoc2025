...
func Compute(positions []Position) int {
	...
	circuits := make([][]string, len(positions))
	for i, p := range positions {
		circuits[i] = []string{
			fmt.Sprintf("%d,%d,%d", p.x, p.y, p.z),
		}
	}

	for i := range len(sortedDistances) {
		...
		if aCircuitIdx != -1 && bCircuitIdx != -1 {
			...
			if i > 10 {
				if len(circuits) == 1 {
					fmt.Println(a, b)
					axString := strings.Split(a, ",")[0]
					ax, _ := strconv.Atoi(axString)
					bxString := strings.Split(b, ",")[0]
